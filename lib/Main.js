...
function game()
{
   ...
   /* var sounds = [
        ...
        {src: "powerup.mp3", id: "powerup"}
    ];*/


    //createjs.Sound.registerSound("pew");
...
function setUpGame(){
    ...
    //timerE = setInterval('drawEnemies()', 1500);
    ...
    stage.addChild(astaroids2,astaroids3,astaroids4, astaroids, pellets, enemyPellets, enemies, powerUps,bossContainer, explosions);
    setUpSpriteSheets();
    ...
    //timerR = setInterval('rotateAstaroid()',400);
    //timerR1 = setInterval('rotateAstaroid2()',450);
    //timerR2 = setInterval('rotateAstaroid3()',475);
    //timerR3 = setInterval('rotateAstaroid4()',425);
...
function bossShoot(){
    if(bossPelletTimer > 40) {



        for(var i = 0; i < bossContainer.children.length; i++)
        {
            enemyPellet = new createjs.Sprite(redshotss);

            enemyPellet.x = bossContainer.children[i].x + 28;
            enemyPellet.y = bossContainer.children[i].y + 21;
            enemyPellet.scaleX = enemyPellet.scaleY = 1.5;
...
function enemyMove(){
    ...
    for (var i = 0; i < enemies.children.length; i++){
        ...
        switch (x) {
            case 1:
                enemies[i].x += Math.floor((Math.random()*10)+1);
            case 2:
                enemies[i].x -= Math.floor((Math.random()*10)+1);
...
function drawBoss(){
    ...
    var boss = new createjs.Bitmap(imagePath + "boss.png");
...
function bossMove(){
    ...
    for (var i = 0; i < bossContainer.children.length; i++){
        ...
        if (leftOrRight == 0) {
            if (bossContainer.children[i].x >= 750) { leftOrRight++;}
            else { bossContainer.children[i].x += 10;}

        }

        if (leftOrRight == 1) {
            if (bossContainer.children[i].x <=50) { leftOrRight--;}
            else { bossContainer.children[i].x -= 10;}
...
function collisionHandler(){
    ...
    for (var k = 0; k < enemies.children.length; k++){
        ...
        if (intersectShip !== null){
            ...
            //xplosion.on("animationend", removeEx(explosion));
    ...
    for (var a = 0; a < bossContainer.children.length; a++) {

        // blueshot collision
        for (var z = 0; z < pellets.children.length; z++){


            var intersectionBoss =  collisionDetector(pellets.children[z], bossContainer.children[a]);

            if (intersectionBoss !== null){

                explosion = new createjs.Sprite(explosionss, "boom");
                explosion.x = bossContainer.children[a].x;
                explosion.y = bossContainer.children[a].y;
                ...
                createjs.Sound.play("explo");
                pellets.removeChildAt(z);
                bossLives--;

                if (bossLives <= 1) {
                    point = point+10;
                    bossContainer.removeChildAt(a);
                }
...
function handleAnimationEnd (e){
    ...
    if(shipDead){
        ...
        /*
        var retryScreen = createjs.Shape();
        retryScreen.graphics.beginFill("#000000").drawRect(0,0,800,800);

        var retry =  new createjs.Text("Retry?","40px Arial", "#FF0000");
        retry.x = 320;
        retry.y = 400;

        retry.addEventListener("click", setUpGame);


        //stage.addChild(retryScreen);
    ...
    switch(enemySpawning) {
        ...
        case 9:
           ...
           if(timerE >=300) {
               ...
               astaroids4.removeAllChildren();
               stage.removeAllChildren();
               bossContainer.removeAllChildren();
               enemySpeed = 2;
               shipDead = false;
               gameRunning = false;
               console.log(stage);
...
function onTick(e)
{
    if(gameRunning){
        ...
        timerA3 = timerA3 +1;
        timerPUP = timerPUP +1;
        bossPelletTimer = bossPelletTimer + 1;
